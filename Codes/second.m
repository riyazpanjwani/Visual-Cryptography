...
share1 = zeros(128,2*128-2);
share2 = zeros(128,2*128-2);
share3 = zeros(128,2*128-2);
...
for i = 1:n
    for j = 1:floor(m/2)
        ...
        if(im1(i,j) == 0)
            share2(i,2*m-2*j-1) = share1(i,2*j);
        else 
            share2(i,2*m-2*j-1) = share1(i,2*j-1);
        end 
        share2(i,2*m-2*j) = 1 - share2(i,2*m-2*j-1);
        
        if(im2(i,m-j+1) == 0)
            share3(i,2*j-1) = share2(i,2*m-2*j);
        else 
            share3(i,2*j-1) = share2(i,2*m-2*j-1);
        end 
            share3(i,2*j) = 1 - share3(i,2*j-1);
        
         if(im3(i,j) == 0)
            share1(i,2*m-2*j-1) = share3(i,2*j);
        else 
            share1(i,2*m-2*j-1) = share3(i,2*j-1);
        end 
        share1(i,2*m-2*j) = 1 - share1(i,2*m-2*j-1);
        
        if(im1(i,m-j+1) == 0)
            share2(i,2*j) = share1(i,2*m-2*j-1);
        else 
            share2(i,2*j) = share1(i,2*m-2*j);
        end 
        share2(i,2*j-1) = 1-share2(i,2*j);
        
        %not clear
        if(im2(i,j) == 0)
            share3(i,2*m-2*j-1) = share2(i,2*j); 
        else 
            share3(i,2*m-2*j-1) = share2(i,2*j-1);
        end 
